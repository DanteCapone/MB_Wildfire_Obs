...
% Load the previous table with significant taxa
% taxa_table_p_vals = readtable('all_taxa_with_p_values.csv');  % Assuming you have already saved the table

% Extract the significant taxa names and their corresponding max CC values from the previous table
significant_taxa = taxa_table_p_vals.Taxa(taxa_table_p_vals.Adjusted_p_value < 0.05);
max_cc_values = taxa_table_p_vals.Max_CC;  % Extract the max cross-correlation coefficients
...
% Set x-axis with ticks every seven days
set(gca, 'XTick', start_date:caldays(10):end_date);
ax = gca;
ax.XTickLabelRotation = 45;  % Rotate labels for readability
...
ftsz = 8;  % Font size
ftname = 'Helvetica';  % Font name
linewdt = 1;  % Line width for plot elements
...
for ii = 1:num_panels
    ...
    yyaxis left
    plot(full_dates, moving_mean_full, 'LineWidth', linewdt+1, 'Color', colorz(ii, :));
    % ylim([0 2e6])
...
ylabel(han_left, 'Taxon Abundance [Cells L^{-1}]', 'FontSize', ftsz, 'FontName', ftname, 'FontWeight', 'bold');
...
ylabel(han_right, 'Total Abundance [Cells L^{-1}]', 'FontSize', ftsz, 'FontName', ftname, 'FontWeight', 'bold', 'Rotation', 270);
