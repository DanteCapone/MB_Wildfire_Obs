...
for i = 1:size(taxa_data, 2)
    ...
    % Cross-correlate
    eval(['[cc(:, i), temp_lags] = xcorr(fillmissing(aod_500nm, ''linear''), ifcbbray.', taxa{i}, ', ''coeff'');']);
...
sc_pm2_5_daily=sc_pm2_5_daily(sc_pm2_5_daily.LocalSiteName=="San Lorenzo Middle School",:);
...
for i = 1:size(taxa_data, 2)
    % Cross-correlation
    eval(['[cc(:, i), temp_lags] = xcorr(fillmissing(pm25, ''linear''), ifcb_pm25.', taxa{i}, ', ''coeff'');']);
    lags(:, i) = temp_lags;
